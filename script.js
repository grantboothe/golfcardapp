...
function buildcard() {
    ...
    for(var bob=18; bob >= 1; bob--){
        ...
        if (title !== 'Out' || title !== 'Total'){
            holecollection += "<div id='column" + bob + "' class='holecol'>"+ bob+"<div>par: "+ objmine.course.holes[(bob - 1)].tee_boxes[0].par+"<div>yards: " + objmine.course.holes[(bob - 1)].tee_boxes[0].yards+""+ "<div>hcp: "+objmine.course.holes[(bob - 1)].tee_boxes[0].hcp+"</div></div></div></div>";
        }
        else {
    ...
    golflist.onreadystatechange = function () {
        if (golflist.readyState == 4 && golflist.status == 200) {
            ...
            for (list = 0; list < myobj.courses.length; list++) {
                console.log(myobj.courses[list].id);
